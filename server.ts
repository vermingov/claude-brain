...
async function handleDesigns(url: URL, req: Request, post: boolean): Promise<Response> {
	...
	if (!post) {
		...
		if (action === "assets" && tail) {
			...
			// A font is always fetched in CORS mode, and the rebuild runs in a sandbox with an opaque
			// origin, so without this every face fails and the page silently falls back to Arial.
			// Fonts only: they are inert, content-addressed and were public where they came from.
